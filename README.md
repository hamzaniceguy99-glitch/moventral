# Moventral — sales coaching

Static marketing site (HTML / CSS / JS, no build step) hosted on GitHub Pages
at `moventral.shop`.

> Generated from `C:\Users\souha\coaching-sites-factory` (content file `sites/moventral.mjs`).
> To change the content, edit that file and run `node build.mjs moventral` — editing the
...
| 🟠 Important | `contact@moventral.shop` doesn't exist yet: set up free email forwarding in Namecheap (*Domain List → Manage → Redirect Email*). | Namecheap |
...
| 🟡 Later | Prices ($59 / $149 / $299) and plan contents should match what you actually sell. | `index.html` `#pricing` |
...
```
Coaching in business-to-business sales, delivered online: one-on-one video sessions and small group workshops covering prospecting and outbound messaging, discovery calls and qualification, demos and proposals, and closing and negotiation. Clients follow a 6 to 12 week program with weekly reviews of their recorded calls and written feedback. No revenue, quota or commission outcome is guaranteed. Services are sold as month-to-month subscriptions from $59 to $299 per month, cancellable at any time. No physical products are sold or shipped. Site: moventral.shop
```
